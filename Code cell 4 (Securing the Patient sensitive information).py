# Securing the Patient sensitive information
import hashlib
import secrets
...
class Patient:
    ...
    # Constructor to initialize patient attributes
    def __init__(self, patient_id, name, dob, gender, address, password):
        self.patient_id = patient_id
        self.name = name
        self.dob = dob
        self.gender = gender
        ...
        self.medical_history = []
        self.current_condition = ""
        self.password_hash = hashlib.sha256(password.encode()).hexdigest()  # Hashing the password

    # Method to verify patient's password
...
class Hospital:
    ...
    # Method to authenticate a patient and return patient object
    ...
    def update_patient(self, patient_id, password, **kwargs):
        patient = self.authenticate_patient(patient_id, password)
        if patient:
            for key, value in kwargs.items():
                setattr(patient, key, value)

    # Method to remove a patient record
    def remove_patient(self, patient_id, password):
        patient = self.authenticate_patient(patient_id, password)
        if patient:
    ...
    def search_patient(self, patient_id, password):
        patient = self.authenticate_patient(patient_id, password)
        if patient:
            doctor = None
...
# To test this code you need to enter the Patient ID and Passwore.
# Patient ID: 1
# Password: password1
# or
# Patient ID: 2¶
...
#Testing the Patient information security
# Creating patients
patient1 = Patient(1, "Aysha Al marri", "1990-05-15", "Female", "123 Main St", "password1")
patient2 = Patient(2, "Mohammed Alblooshi", "1985-10-20", "Male", "456 Dubai St", "password2")
# Creating doctors
...
hospital.update_patient(1, "password1", address="456 Updated St")
...
# Prompting the user to enter the password to view patient summary
patient_id = int(input("Enter your patient ID: "))
password = input("Enter your password: ")

# Searching for a patient and displaying their summary
patient_summary = hospital.search_patient(patient_id, password)
if patient_summary:
    print("Patient Summary:")
    for key, value in patient_summary.items():
        print(key + ":", value)
else:
    print("Patient not found or invalid password.")
